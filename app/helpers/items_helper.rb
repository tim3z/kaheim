module ItemsHelper
  def badges_for item
    ...
    result << content_tag(:span, t('helpers.hidden'), class: 'label label-default') << ' ' unless item.active?
    ...
    result << content_tag(:span, t('items.locked'), class: 'label label-danger') unless item.user.unlocked?
    result.html_safe
  end
